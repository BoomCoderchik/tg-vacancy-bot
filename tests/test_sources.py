...
def test_build_adapters_registers_guest_posts_independent_of_headless() -> None:
    settings = Settings(
        TELEGRAM_BOT_TOKEN="token",
        TARGET_CHAT_ID="@target",
        ENABLE_LINKEDIN_POST_SEARCH=False,
    ...
    assert [adapter.name for adapter in build_adapters(settings)] == ["LinkedIn Posts (Guest)"]
