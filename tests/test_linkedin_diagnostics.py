...
def test_diagnostics_continues_after_provider_error_and_deduplicates(monkeypatch) -> None:
    ...
    assert "provider=serpapi status=error candidates=0 error_type=RuntimeError" in output
    assert "provider=serper status=ok candidates=1" in output
...
def test_diagnostics_distinguishes_empty_and_all_provider_errors(monkeypatch) -> None:
    ...
    settings = Settings(SERPAPI_API_KEY="secret", SERPER_API_KEY="")
