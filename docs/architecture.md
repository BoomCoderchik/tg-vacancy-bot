...
Beyond the LinkedIn family, two opt-in Russia-wide sources share the same publication pipeline:
...
Like every source, both Russia-wide adapters pass vacancies through the unified vacancy filtering policy, freshness filter (`SOURCE_MAX_AGE_HOURS` for dated results; ordering + deduplication for undated ones), localization boundary, publication limit, and SQLite deduplication before Telegram publication.
