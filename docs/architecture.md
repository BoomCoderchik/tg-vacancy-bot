...
- `tg_vacancy_bot/config.py`
  ...
  - Supports opt-in Russia-wide vacancy search with `ENABLE_RUSSIA_SEARCH`, `RUSSIA_SEARCH_QUERY` (blank = auto-built from the active `/filters` selection), `RUSSIA_SEARCH_RESULTS_WANTED`, and `RUSSIA_SEARCH_PROVIDERS`.
  - Supports opt-in public Telegram vacancy channels with `ENABLE_RUSSIA_TELEGRAM`, `RUSSIA_TELEGRAM_CHANNELS` (comma-separated public channel usernames), and `RUSSIA_TELEGRAM_MAX_POSTS_PER_CHANNEL`.
  - Supports opt-in HeadHunter official RSS search feed with `ENABLE_HHRU_RSS`, `HHRU_RSS_QUERY` (blank = auto-built from the active `/filters` selection), and `HHRU_RSS_RESULTS_WANTED`.
...
- `tg_vacancy_bot/sources/`
  - Source adapter package for LinkedIn hiring-post discovery and Russia-wide open-web vacancy discovery.
  - Registers keyed search, free search-result scraping, or permission-gated headless LinkedIn post parsing according to the configured opt-in flags, plus the Russia internet-search and public Telegram-channel adapters when their flags are enabled.
  - Every automatic LinkedIn vacancy needs a reliable publication date before it can reach source polling; the Russia-wide sources follow the base-layer freshness behavior (dated results filtered by `SOURCE_MAX_AGE_HOURS`, undated results handled by ordering and deduplication).
  - `sources/search_providers.py` holds the shared free public search pipeline (Bing RSS, DuckDuckGo HTML, Bing HTML, DuckDuckGo Lite, Mojeek) reused by the LinkedIn scraper/headless adapters and the Russia internet-search adapter.
...
- Russia internet-search is controlled by `ENABLE_RUSSIA_SEARCH=false` by default and needs no key. It reads the same free providers as the LinkedIn scraper and keeps every real `http(s)` page except search-engine domains and LinkedIn; Russian job domains are prioritized for ordering only. Anti-bot challenge pages are skipped rather than bypassed.
- Public Telegram vacancy channels are controlled by `ENABLE_RUSSIA_TELEGRAM=false` by default and need no key or account. The adapter reads each configured public channel's `https://t.me/s/<username>` preview page and skips private or blocked channels instead of bypassing them.
- HeadHunter official RSS search is controlled by `ENABLE_HHRU_RSS=false` by default and needs no key or account. The adapter reads `https://hh.ru/search/vacancy/rss` with web-browser headers only, splits `HHRU_RSS_QUERY` on `||`, auto-builds queries from the active `/filters` selection when the query is empty, reads each item's real `pubDate` (with the `Создана:` field as a fallback), maps company/region/salary from the description, and isolates query failures so the remaining queries still run.
...
The project permits five automatic LinkedIn adapters across five opt-in paths:
...
- `LinkedInPostGuestAdapter`, off by default for the bot-target pipeline and enabled only with `ENABLE_LINKEDIN_POST_GUEST=true`, discovers public post URLs through the same free public search providers as the scraper, then reads each post's own linkedin.com page through ordinary guest HTTP: real post text plus an activity-ID publication date. A login-walled `/posts/` URL is retried once through its public feed-update form, and a still-blocked post is published from the real, dated public search result that discovered it. It needs no account, key, or browser.

Beyond the LinkedIn family, three opt-in Russia-wide sources share the same publication pipeline:

- `RussiaVacancySearchAdapter`, enabled only with `ENABLE_RUSSIA_SEARCH=true`, searches the whole open web through the free public search providers used by the LinkedIn scraper with no `site:` restriction, prioritizes Russian job domains for ordering, auto-builds queries from the active `/filters` selection when `RUSSIA_SEARCH_QUERY` is empty, skips challenge pages (never bypasses), and passes undated results to the base layer.
- `TelegramVacancyChannelAdapter`, enabled only with `ENABLE_RUSSIA_TELEGRAM=true` and a non-empty `RUSSIA_TELEGRAM_CHANNELS` list, reads each configured public channel's public `t.me/s/<username>` preview page and maps public posts into `Vacancy` cards. No account login, password, CAPTCHA handling, or protection bypass is involved; private or blocked channels are skipped.
- `HeadHunterRssAdapter`, enabled only with `ENABLE_HHRU_RSS=true`, reads HeadHunter's official public RSS search feed at `https://hh.ru/search/vacancy/rss` with plain web-browser headers, one auto-built query per selected specialty × grade pair (Russian and English) spliced on the feed's `OR` operator, maps each item's real publication date and company/region/salary, and feeds everything into the common pipeline. No API key, account, or protection bypass is involved.

Like every source, all three Russia-wide adapters pass vacancies through the unified vacancy filtering policy, freshness filter (`SOURCE_MAX_AGE_HOURS` for dated results; ordering + deduplication for undated ones), localization boundary, publication limit, and SQLite deduplication before Telegram publication.
